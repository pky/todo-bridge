...
function acceptanceLabel(type: DocumentSuggestion['type']): string {
  if (type === 'task') return 'Todo候補として採用'
  if (type === 'calendar_event') return '予定候補として採用'
  return '情報として採用'
}
</script>
...
<template>
  <section data-testid="document-suggestions" class="mt-4 rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
    <div class="flex items-center justify-between gap-3">
      ...
      <span v-if="suggestions.length" class="shrink-0 text-xs text-slate-500">{{ suggestions.length }}件</span>
    </div>
    ...
    <p v-else-if="suggestions.length === 0" class="mt-3 text-sm text-slate-500">この書類から確認候補は見つかりませんでした。</p>
      ...
      >
        ...
        <div class="mt-3 grid gap-2 sm:grid-cols-3">
          ...
          <button
            ...
            @click="save(suggestion, 'accepted')"
          >{{ acceptanceLabel(suggestion.type) }}</button>
          <button
